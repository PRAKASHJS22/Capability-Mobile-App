...
stopVideo= async()=>{
  ...
  renderBarcode = ({ bounds, data, type }) => (
    <React.Fragment key={data + bounds.origin.x}>
      ...
      >
        <Text style={[styles.textBlock]}>{`${data} ${type}`}</Text>{alert(`${type}${data}`)}
      </View>
    </React.Fragment>
  );
  ...
  renderCamera() {
    ...
    return (
      ...
      >
        ...
        >
          <Slider
            style={{ width: 150, marginTop: 15, alignSelf: 'flex-end',marginBottom:20 }}
        ...
        >
          ...
          >
            {this.state.isRecording ? (
              <Text style={styles.flipText}> Recording </Text>
            ) : (
